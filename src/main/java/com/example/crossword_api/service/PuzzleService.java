...
import com.example.crossword_api.domain.Board;
import com.example.crossword_api.domain.Caption;
import com.example.crossword_api.domain.Cell;
...
public class PuzzleService {
  ...
  @Transactional
  public void createPuzzle(LocalDate publishDate) {
    ...
    for (int z = 0; z < board.getColCount(); z++) {
      Cell cell = new Cell();
    ...
    for (int i = 1; i < words.size(); i++) {
      ...
      for (int r = 0; r < board.getRowCount(); r++) {
        for (int c = 0; c < board.getColCount(); c++) {
          ...
          // let's compare words
          iteratingLetters:
          for (int l = 0; l < word.getName().length(); l++) {
    ...
    PuzzleData puzzleData = new PuzzleData(board.getGrid(), captions);

    Puzzle puzzle = new Puzzle(puzzleData, publishDate);
